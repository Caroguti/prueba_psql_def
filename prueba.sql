...
SELECT nombre_cliente, precio_total 
FROM facturas 
INNER JOIN clientes 
ON facturas.id_cliente = clientes.id_cliente 
WHERE precio_total=(SELECT max(precio_total) 
FROM facturas);
...
SELECT nombre_cliente, precio_total FROM facturas 
INNER JOIN clientes 
ON facturas.id_cliente = clientes.id_cliente  
WHERE precio_total > 10;
...
INNER JOIN facturas ON detalle_factura.id_producto = 'F002P003'
) AS n;
